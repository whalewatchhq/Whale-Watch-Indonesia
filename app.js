...
function WhaleWatchApp() {
  ...
  // Fungsi baru untuk menampilkan artikel dan mengatur URL Hash
  const handleViewArticle = (article) => {
    setSelectedArticle(article);
    window.location.hash = `#article/${article.id}`;
  };
  
  // Placeholder fungsi subscribe
  ...
  // 1. Load articles pada aplikasi pertama kali dimuat
  useEffect(() => {
    loadArticles();
  }, []);

  // 2. Baca URL Hash setelah articles dimuat (untuk deep link)
  useEffect(() => {
    if (articles.length > 0) {
      ...
      // 3. Listener untuk tombol Back/Forward browser
          ...
          <button
            onClick={() => {
                ...
                navigator.clipboard.writeText(shareUrl)
                    ...
                    .catch(() => alert("Gagal menyalin link. Silakan salin manual dari address bar."));
            }}
  ...
  />
      ...
      {topNews.length > 0 && (
        <div className="p-4">
          ...
          {/* PERUBAHAN UTAMA DI SINI: untuk Horizontal Scrolling */}
          <div className="flex overflow-x-scroll space-x-4 pb-4 snap-x snap-mandatory">
            {topNews.map((t) => (
              <div
                key={t.id}
                // Tambahkan width, flex-shrink, dan snap-center untuk slider
                className="rounded-lg overflow-hidden cursor-pointer w-[85vw] flex-shrink-0 snap-center" 
                onClick={() => handleViewArticle(t)} // Menggunakan fungsi routing baru
              >
                <img
                  src={t.thumbnail}
        ...
        {filteredArticles.length === 0 ? (
          ...
          <div className="space-y-4">
            {filteredArticles.map((a) => (
              <div
                key={a.id}
                onClick={() => handleViewArticle(a)} // Menggunakan fungsi routing baru
